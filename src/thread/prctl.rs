...
pub fn set_keep_capabilities(enable: bool) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_KEEPCAPS, ptr::invalid_mut(enable as usize)) }.map(|_r| ())
}
...
pub fn set_no_new_privs(no_new_privs: bool) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_NO_NEW_PRIVS, ptr::invalid_mut(no_new_privs as usize)) }
        .map(|_r| ())
}
...
pub fn disable_transparent_huge_pages(thp_disable: bool) -> io::Result<()> {
    unsafe { prctl_2args(PR_SET_THP_DISABLE, ptr::invalid_mut(thp_disable as usize)) }.map(|_r| ())
}
